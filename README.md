...
在飞牛系统上安装本插件后，**无需开启远程访问**，即可把聊天图片上传到飞牛 NAS。

- 第一步：在飞牛应用商店安装 AstrBot。
- 第二步：在飞牛 Photos 中新建一个文件夹用来存放上传的照片（也可以直接用 Photos，建议新建文件夹）。
- 第三步：安装本插件，并配置图片保存地址为你需要的地址。
- 第四步：给文件夹开放写入权限：

1. 确认 AstrBot 运行用户（原生安装/非容器时尤其重要）：
   ```
